...
export default function ESGForm({ schema }) {
  ...
  const seedValuesFromMapOnce = useCallback((map) => {
    // Only seed blanks/undefined; do NOT clobber user typing
    setValues((prev) => {
      const next = { ...prev };
      Object.entries(map).forEach(([name, info]) => {
        ...
        // also seed KPI toggle default if we haven't seen it yet
        const kpiKey = `${name}_isKpi`;
        if (next[kpiKey] === undefined) {
          next[kpiKey] = false; // default unchecked; user can toggle
        }
      });
      return next;
    });
